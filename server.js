...
// ─────────────────────────────────────────────────────────────────────────────
// ── WATCH PARTY — WebSocket room server ──────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
 ...
 *   code: "ABC123",
 *   contentId: string,
 *   passwordHash: string | null,   // SHA-256 hex, or null for open rooms
 *   hostId: string,                // ws client id
 *   participants: Map<id, { name, ws }>
 * }
 */
const watchPartyRooms = new Map(); // code → room
const clientRooms     = new Map(); // wsId → code (which room this client is in)
...
function generateRoomCode() {
    ...
    } while (watchPartyRooms.has(code));
    return code;
}
...
function participantsList(room) {
    return Array.from(room.participants.values()).map(p => p.name);
}
...
function removeClientFromRoom(wsId) {
    const code = clientRooms.get(wsId);
    if (!code) return;
    const room = watchPartyRooms.get(code);
    if (!room) return;

    const leaving = room.participants.get(wsId);
    room.participants.delete(wsId);
    clientRooms.delete(wsId);

    if (room.participants.size === 0) {
        // Last person left → destroy room
        watchPartyRooms.delete(code);
        return;
    }

    // If host left, assign a new host (first remaining participant)
    if (room.hostId === wsId) {
        room.hostId = room.participants.keys().next().value;
        broadcastRoom(room, { type: 'system', text: `👑 ${room.participants.get(room.hostId).name} ahora es el host.` });
    }

    if (leaving) {
        broadcastRoom(room, { type: 'system', text: `🚪 ${leaving.name} salió de la sala.` });
    }
    broadcastRoom(room, {
        type: 'participants_update',
        count: room.participants.size,
        names: participantsList(room)
    });
}

// Attach WebSocket server to the same HTTP server
const server = app.listen(PORT, '0.0.0.0', () => console.log(`Servidor activo en puerto ${PORT}`));

const wss = new WebSocketServer({ server, path: '/watchparty/ws' });
    ...
    ws.on('message', (raw) => {
        ...
        if (action === 'create_room') {
            ...
            const room = {
                ...
                passwordHash: hashPassword(msg.password),
                hostId:       wsId,
                participants: new Map()
            };
            const name = (msg.name || 'Host').substring(0, 32);
            room.participants.set(wsId, { name, ws });
            watchPartyRooms.set(code, room);
            clientRooms.set(wsId, code);
            ...
            console.log(`[WatchParty] Sala creada: ${code} (host: ${name})`);
            ...
            if (!room) {
                return sendToClient(ws, { type: 'error', message: 'Sala no encontrada. Verifica el código.' });
            }
            if (room.passwordHash && hashPassword(msg.password) !== room.passwordHash) {
                return sendToClient(ws, { type: 'error', message: 'Contraseña incorrecta.' });
            }
            if (room.participants.size >= 20) {
                return sendToClient(ws, { type: 'error', message: 'La sala está llena (máx 20 personas).' });
            }
            // Kick previous connection if same wsId somehow re-joins
            if (clientRooms.has(wsId)) removeClientFromRoom(wsId);

            const name = (msg.name || 'Invitado').substring(0, 32);
            room.participants.set(wsId, { name, ws });
            clientRooms.set(wsId, code);
            ...
            console.log(`[WatchParty] ${name} se unió a sala ${code} (${room.participants.size} personas)`);
            ...
            if (!code) return;
            const room = watchPartyRooms.get(code);
            if (!room || room.hostId !== wsId) return; // only host can sync

            broadcastRoom(room, {
                type:       'sync',
                syncAction: msg.syncAction || 'play',
                positionMs: msg.positionMs || 0
            }, wsId); // exclude host from echo
            ...
            const room = watchPartyRooms.get(code);
            if (!room) return;
            const participant = room.participants.get(wsId);
            if (!participant) return;

            const text = (msg.text || '').substring(0, 500).trim();
            if (!text) return;
            ...
            broadcastRoom(room, {
                ...
                text
            }); // send to everyone including sender (echo back)
        }
    });

    ws.on('close', () => {
        removeClientFromRoom(wsId);
    });
